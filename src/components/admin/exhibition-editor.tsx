  ...
  return (
    <form action={saveExhibitionAction} className="mt-5 grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <TextEditor label="Ano" value={data.year as string} onChange={(val) => update("year", val)} />
        <TextEditor label="Selo da Edição" value={data.editionLabel as string} onChange={(val) => update("editionLabel", val)} />
        ...
        <div className="md:col-span-2">
          <ListEditor label="Localização" items={data.location as string[]} onChange={(val) => update("location", val)} />
        </div>
